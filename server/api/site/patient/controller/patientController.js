...
console.log('Phase Patient call............');
...
console.log(' Get Phase Patient...............');
 db.models.user
    .findAll(
        {group:groupval,
         having:Sequelize.literal('(`patients.phase_patients.phase.trial`.`id` IS NULL) OR (COUNT(`user`.`id`) >= 1 AND `patients.phase_patients.phase.trial`.`active` = 1 AND `patients.phase_patients.phase.trial`.`id` = "'+ trial_Id +'") OR (COUNT(`user`.`id`) = 1 AND `patients.phase_patients.phase.trial`.`active` = 2)'),
    ...
    db.query("SELECT `user`.`id`,`user`.`email`,`user`.`firstname`,`user`.`lastname`,`patients`.`id` AS patientId,`patients`.`user_id` AS userId,`patients`.`age`,`patients`.`gender`,`patients`.`placebo` FROM `user` AS `user` INNER JOIN `patient` AS `patients` ON `user`.`id` = `patients`.`user_id` WHERE `user`.`deletedAt` IS NULL and `patients`.`id` not in (select distinct phase_patient.patient_id FROM `phase_patient` INNER JOIN `phase` ON `phase_patient`.`phase_id` = `phase`.`id` INNER JOIN `trial` ON `phase`.`trial_id` = `trial`.`id` AND (`trial`.active = 1 and trial.id != '"+ trial_Id +"'))",
    {
      type: sequelizeDb.QueryTypes.SELECT
    }).then(function(patient) {
      console.log(patient);
      console.log('Phase Patient End...................');
      if(patient) {
        let patientArray = [];
          ...
          async.forEach(Object.keys(patient), function (item, callback1)
           ...
           let patientList = 
                          {
                            "firstname" : patient[item].firstname,
                            "lastname" : patient[item].lastname,
                            "email" : patient[item].email,
                            "user_id" : patient[item].userId,
                            "patients" : [{
                                            "id" : patient[item].patientId,
                                            "age" : patient[item].age,
                                            "gender" : patient[item].gender
                                        }]
                          };
...
db.models.user
    .findOne({where:{id : userInfo.id},attributes:['id','role_id','timezone']})
    .then(function(user) {
        console.log('User Found.............');
        console.log(user.dataValues.timezone);
        let role_id = user.dataValues.role_id;
        let user_timezone = user.dataValues.timezone;

        var currentDate = generalConfig.convertUTCDate(new Date(), user_timezone);
        console.log('get Fallout Patient .............');
    ...
    .findAll(
        {
         group:groupval,
         having:Sequelize.literal('`patients.vital_dosage_statuses`.`schedule_on` <=  "'+ moment.utc(new Date()).format("YYYY/MM/DD HH:mm:ss") +'"  and `patients.vital_dosage_statuses.phase.trial`.`active` = 1 and `patients.vital_dosage_statuses`.`type` = 2 and ('+ role_id +' = 38 or ('+ role_id +' = 39 and `patients.vital_dosage_statuses.phase.trial`.`croCoordinator_id`= "'+ userInfo.id +'") or ('+ role_id +' = 40 and `patients.vital_dosage_statuses.phase.trial`.`dsmb_id`= "'+ userInfo.id +'"))'),
         order:orderval,
         attributes:['id','email','firstname','lastname'],
         include : [{model : db.models.patient,attributes:['id','user_id','age','gender','placebo'],required:true,
         include: [{ model: db.models.vital_dosage_status, attributes: ['id','phase_id','patient_id','schedule_on','status','trial_id','type'],
                     include : [{model : db.models.phase,
                                 attributes:['id','trial_id','start_date','tentitive_end_date','sr_no'],
                                 where : {
                                    $and: {
                                      start_date: {
                                        $lte: new Date()
                                 ...
                                 include :[{model : db.models.trial,attributes:['id','name','active','croCoordinator_id','dsmb_id'],
...
exports.getEnrolledPhasePatientsTrialsDSMB = function(req, res, next) {
  ...
  db.query("SELECT  `trial`.`id`, `trial`.`name`, `trial`.`active`, `phases`.`id` AS `phases.id`, `phases`.`trial_id` AS `phases.trial_id`, `phases`.`sr_no` AS `sr_no`, `phases`.`description` AS `phases.description`, `phases`.`start_date` AS `phases.start_date`, `phases`.`tentitive_end_date` AS `phases.tentitive_end_date`, `phases`.`participant_count` AS `phases.participant_count`, `phases`.`active` AS `phases.active`, `phases`.`createdAt` AS `createdAt`, `phases`.`updatedAt` AS `phases.updatedAt`, `phases.phase_patients`.`id` AS `phases.phase_patients.id`, `phases.phase_patients`.`phase_id` AS `phase_id`, `phases.phase_patients`.`patient_id` AS `patient_id`, `phases.phase_patients.patient`.`id` AS `phases.phase_patients.patient.id`, `phases.phase_patients.patient`.`user_id` AS `phases.phase_patients.patient.user_id`, `phases.phase_patients.patient`.`age` AS `age`, `phases.phase_patients.patient`.`gender` AS `gender`,`phases.phase_patients.patient`.`placebo` AS `placebo`, `phases.phase_patients.patient.user`.`id` AS `phases.phase_patients.patient.user.id`, `phases.phase_patients.patient.user`.`firstname` AS `firstname`, `phases.phase_patients.patient.user`.`lastname` AS `lastname` FROM `trial` AS `trial`     INNER JOIN `phase` AS `phases` ON `trial`.`id` = `phases`.`trial_id`     AND `phases`.`start_date` < '" + newDate + "'     AND `phases`.`tentitive_end_date` > '" + newDate + "'     AND `phases`.`active` = true     INNER JOIN `phase_patient` AS `phases.phase_patients` ON `phases`.`id` = `phases.phase_patients`.`phase_id`     AND `phases.phase_patients`.`phase_id` IS NOT NULL     AND `phases.phase_patients`.`patient_id` IS NOT NULL     LEFT OUTER JOIN `patient` AS `phases.phase_patients.patient` ON `phases.phase_patients`.`patient_id` = `phases.phase_patients.patient`.`id`     LEFT OUTER JOIN `user` AS `phases.phase_patients.patient.user` ON `phases.phase_patients.patient`.`user_id` = `phases.phase_patients.patient.user`.`id`     AND `phases.phase_patients.patient.user`.`deletedAt` IS NULL   WHERE `trial`.`active` = 1 AND `trial`.`dsmb_id` = '" + dsmbId +  "' " + wheretrialcon + " " + wheregendercon+ " LIMIT " + pageSize + " OFFSET " + offset1,
{
      type: sequelizeDb.QueryTypes.SELECT
    }).then(function(patient) {
      if(patient) {
...
exports.getEnrolledPhasePatientsTrialsCROCo = function(req, res, next) {

  var croCo = generalConfig.getUserInfo(req);
  var croCoId = croCo.id;
  
  console.log('Patient Trial Call ...........Start');
  console.log(croCoId);
  console.log('Patient Trial Call ........... End');
  ...
  db.query("SELECT  `trial`.`id`, `trial`.`name`, `trial`.`active`, `phases`.`id` AS `phases.id`, `phases`.`trial_id` AS `phases.trial_id`, `phases`.`sr_no` AS `sr_no`, `phases`.`description` AS `phases.description`, `phases`.`start_date` AS `phases.start_date`, `phases`.`tentitive_end_date` AS `phases.tentitive_end_date`, `phases`.`participant_count` AS `phases.participant_count`, `phases`.`active` AS `phases.active`, `phases`.`createdAt` AS `createdAt`, `phases`.`updatedAt` AS `phases.updatedAt`, `phases.phase_patients`.`id` AS `phases.phase_patients.id`, `phases.phase_patients`.`phase_id` AS `phase_id`, `phases.phase_patients`.`patient_id` AS `patient_id`, `phases.phase_patients.patient`.`id` AS `phases.phase_patients.patient.id`, `phases.phase_patients.patient`.`user_id` AS `phases.phase_patients.patient.user_id`, `phases.phase_patients.patient`.`age` AS `age`, `phases.phase_patients.patient`.`gender` AS `gender`,`phases.phase_patients.patient`.`placebo` AS `placebo`, `phases.phase_patients.patient.user`.`id` AS `phases.phase_patients.patient.user.id`, `phases.phase_patients.patient.user`.`firstname` AS `firstname`, `phases.phase_patients.patient.user`.`lastname` AS `lastname` FROM `trial` AS `trial`     INNER JOIN `phase` AS `phases` ON `trial`.`id` = `phases`.`trial_id`     AND `phases`.`start_date` < '" + newDate + "'     AND `phases`.`tentitive_end_date` > '" + newDate + "'     AND `phases`.`active` = true     INNER JOIN `phase_patient` AS `phases.phase_patients` ON `phases`.`id` = `phases.phase_patients`.`phase_id`     AND `phases.phase_patients`.`phase_id` IS NOT NULL     AND `phases.phase_patients`.`patient_id` IS NOT NULL     LEFT OUTER JOIN `patient` AS `phases.phase_patients.patient` ON `phases.phase_patients`.`patient_id` = `phases.phase_patients.patient`.`id`     LEFT OUTER JOIN `user` AS `phases.phase_patients.patient.user` ON `phases.phase_patients.patient`.`user_id` = `phases.phase_patients.patient.user`.`id`     AND `phases.phase_patients.patient.user`.`deletedAt` IS NULL   WHERE `trial`.`active` = 1 AND `trial`.`croCoordinator_id` = '" + croCoId +  "' " + wheretrialcon + " " + wheregendercon+ " LIMIT " + pageSize + " OFFSET " + offset1,
{
      type: sequelizeDb.QueryTypes.SELECT
    }).then(function(patient) {
      if(patient) {
        db.query("SELECT  count(*) AS totalCount  FROM `trial` AS `trial`     INNER JOIN `phase` AS `phases` ON `trial`.`id` = `phases`.`trial_id`     AND `phases`.`start_date` < '" + newDate + "'     AND `phases`.`tentitive_end_date` > '" + newDate + "'     AND `phases`.`active` = true     INNER JOIN `phase_patient` AS `phases.phase_patients` ON `phases`.`id` = `phases.phase_patients`.`phase_id`     AND `phases.phase_patients`.`phase_id` IS NOT NULL     AND `phases.phase_patients`.`patient_id` IS NOT NULL     LEFT OUTER JOIN `patient` AS `phases.phase_patients.patient` ON `phases.phase_patients`.`patient_id` = `phases.phase_patients.patient`.`id`     LEFT OUTER JOIN `user` AS `phases.phase_patients.patient.user` ON `phases.phase_patients.patient`.`user_id` = `phases.phase_patients.patient.user`.`id`     AND `phases.phase_patients.patient.user`.`deletedAt` IS NULL WHERE `trial`.`active` = 1 AND `trial`.`croCoordinator_id` = '" + croCoId +  "' " + wheretrialcon + " " + wheregendercon+ " ",{
      type: sequelizeDb.QueryTypes.SELECT
    }).then(function(data){
      console.log('Count................');
...
exports.getEnrolledTrialsCROCo = function(req, res, next) {
