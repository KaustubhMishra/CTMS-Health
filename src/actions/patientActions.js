...
export function loadEnrolledPatientTrialCROCo(search) {
  return function (dispatch) {
    dispatch(beginAjaxCall());
    return PatientApi.getEnrolledPatientTrialCROCo(search).then(enrolledTrialPatientData => {
      dispatch(loadEnrolledPatientList(enrolledTrialPatientData));
    }).catch(error => {
      throw(error);
    });
...
export function loadEnrolledTrialsCROCo() {
  return function (dispatch) {
    dispatch(beginAjaxCall());
    return PatientApi.getEnrolledTrialCROCo().then(enrolledTrialPatientCRO => {
      dispatch(loadEnrolledPatientTrials(enrolledTrialPatientCRO));
    }).catch(error => {
      throw(error);
    });
